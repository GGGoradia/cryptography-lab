# AES-128 implementation in Python (no libraries)
...
def shift_rows(s): return [s[i] for i in [0, 5, 10, 15, 4, 9, 14, 3, 8, 13, 2, 7, 12, 1, 6, 11]]
def inv_shift_rows(s): return [s[i] for i in [0, 13, 10, 7, 4, 1, 14, 11, 8, 5, 2, 15, 12, 9, 6, 3]]
...
def pad_block(s): return list(s.encode().ljust(16, b'\x00'))
def unpad_block(b): return bytes(b).rstrip(b'\x00').decode()

def aes_encrypt(text, keytext):
    block = pad_block(text)
    key = pad_block(keytext)
    encrypted = aes_encrypt_block(block, key)
    return encrypted

def aes_decrypt(cipher_block, keytext):
    key = pad_block(keytext)
    decrypted_block = aes_decrypt_block(cipher_block, key)
    return unpad_block(decrypted_block)

if __name__ == "__main__":
    plaintext = "aesdemo_text"
    key = "64bit_keydemo"

    cipher_block = aes_encrypt(plaintext, key)
    cipher_hex = ''.join(format(b, '02x') for b in cipher_block)
    decrypted = aes_decrypt(cipher_block, key)
